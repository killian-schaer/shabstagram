...
- **`AUTH_KILLSWITCH`** : si `true`, contourne entièrement la connexion
  professionnelle et ouvre une session d'administration factice, sans jamais
  créer d'utilisateur réel en base. Réservé au développement local — ne
...
- **Inscription d'application Entra ID** : une seule inscription suffit. Elle
  ...
  `openid`, `profile`, `email`, `User.Read`) et à l'envoi de courriels /
  recherche d'annuaire via un jeton applicatif (permissions **applicatives**
  `Mail.Send` et `User.Read.All`, à consentir par un administrateur du
  tenant). Renseigner `ENTRA_TENANT_ID`, `ENTRA_CLIENT_ID`,
